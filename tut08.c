...
int audio_decode_frame(VideoState *is, uint8_t *audio_buf, int buf_size, double *pts_ptr)
{
    ...
    while (response >= 0)
    {
        ...
        // will always call av_frame_unref(frame) before doing anything else
        // so we don't need to free after since we use a static AVFrame*
        // technically only on quit but then all the memory is freed anyway
...
{
    // for outputting silence while paused / if not using SDL_PauseAudioDevice
    // if(global_video_state->paused)
    // {
    //     // output 1024bytes of silence
    //     memset(stream, 0, len);
    //     return;
    // }
...
int stream_component_open(VideoState *is, int stream_index)
{
    ...
    switch(codecParameters->codec_type)
    {
        case AVMEDIA_TYPE_AUDIO:
        {
            ...
            // uint32
            is->audio_device_id = SDL_OpenAudioDevice(
                    NULL, 0,
                    &wanted_spec, &obtained_spec,
                    SDL_AUDIO_ALLOW_FORMAT_CHANGE);
            if (is->audio_device_id == 0)
            {
            ...
            SDL_PauseAudioDevice(is->audio_device_id, 0);
        } break;
...
int decode_thread(void *arg)
{
    ...
    for (;;)
    {
        ...
        // NOTE: decoding ahead till we reace MAX_XXX_SIZE even when paused, pausing ony means that
        // audio/video don't take any packets of the queue
...
// TODO Fullscreen option, Frame dropping (instead of short refresh)
            ...
            case SDL_KEYDOWN:
            {
                ...
                switch(event.key.keysym.sym)
                {
                    ...
                    case SDLK_SPACE:
                    {
                        ...
                        if(is->paused)
                        {
                            // need to correct frame_timer when unpausing since it's
                            // supposed to represent what time it should be when
                            // we show the next frame (compared to computer clock)
                            // correct by using the computer clock time of our last frame
                            // (which is still in microseconds)
                            is->frame_timer += (av_gettime() - is->video_current_pts_time) /
                                1000000.0;
                        }
                        is->paused = !is->paused;
                        // 0 unpause, non-zero pause
                        SDL_PauseAudioDevice(is->audio_device_id, is->paused);
                    } break;
